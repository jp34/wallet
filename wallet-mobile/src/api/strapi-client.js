...
export const createAccount = async (username, email, password, agreement) => {
  try {
    if (!agreement)
      throw new Error("User has not accepted the terms and conditions!");
    const response = await axios.post(
      baseUrl.concat("/api/auth/local/register"),
      {
        username: username,
        email: email,
        password: password,
...
export const createPatientAllergy = async (
  ...
  try {
    if (!session) throw Error("User has not been authenticated yet");
    const response = await axios.post(
      baseUrl.concat("/api/patient-allergies"),
      {
        data: {
          patient: patientId,
...
export const createPatientMedication = async (
  ...
  try {
    if (!session) throw Error("User has not been authenticated yet");
    const response = await axios.post(
      baseUrl.concat("/api/patient-medications"),
      {
        data: {
          patient: patientId,
          name: name,
...
export const getPatientData = async () => {
  try {
    ...
    const response = await axios.get(
      baseUrl.concat(`/api/findByUsername/${session.user.username}`),
      {
        headers: {
          Authorization: `Bearer ${session.token}`,
        },
